"use strict";
var app = angular.module( 'mowner-app', [ 'ngRoute', 'angular-loading-bar', 'localytics.directives', 'ngMaterial' ] );
app.config( [ '$routeProvider', 'cfpLoadingBarProvider',
    function ( $routeProvider, cfpLoadingBarProvider ) {
        $routeProvider.
        when( '/', {
            templateUrl: '../public/app/mowner/index.php'
        } ).
        otherwise( {
            redirectTo: '/'
        } );
...
function IndexCTL( $scope, $compile, $http, $timeout, $q, $log ) {
    ...
    $scope.deleteOwners = function () {
        if ( !window.confirm( 'Are you sure?' ) ) return;
        if ( s.item_id != 'new' ) {
            $.post( "../api/owner/delete/" + s.item_id, {}, function ( data ) {
                if ( data.error ) {
                    alert( data.error.message );
                    return;
                }
    ...
    function setToEditOwner( item ) {
        ...
        */
        ...
        $http.get( '../api/mowner/'+item.id ).success( function ( data ) {
            //self.states = data;
            $scope.owner_lists = data;
        } );
