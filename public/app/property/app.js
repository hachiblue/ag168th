...
var app = angular.module( 'property-app', [ 'ngRoute', 'angular-loading-bar', 'localytics.directives', 'ngMaterial' ] );
...
app.controller( 'ListCTL', [ '$scope', '$http', '$location', '$route', function ( $scope, $http, $location, $route )
{
    ...
    {
        if ( $index < 1 || $index > $scope.pagination.length ) return;
...
app.controller( 'AddCTL', [ '$scope', '$compile', '$http', '$location', '$q', '$log', function ( $scope, $compile, $http, $location, $q, $log )
{
    var self = this;
    ...
    function querySearch( query )
    {
        ...
        $http.get( url ).success( function ( data )
        {
            ...
            self.selectedItemChange( data[0] );
    ...
    {
        s.form.owner_id = null;
        if ( item )
        {
            s.form.owner_id = item.id;
    ...
    $scope.formProjectIdChange = function ( )
    {
        ...
        if ( $scope.form.project_id )
        {
            project = ( function ( )
            {
                ...
                {
                    if ( $scope.collection.project[ i ].id == $scope.form.project_id ) return $scope.collection.project[ i ];
                }
                return false;
            } )( );
        }
    ...
    $scope.submit = function ( )
    {
        ...
        {
            alert( "please select your owner" );
            return;
        }
    ...
    $scope.addmore_owner = function ( )
    {
        ...
        html = '<div class="row" id="row_' + this.ctn + '">' + tmpl.html( ).replace( /owner_name1/g, 'owner_name' + this.ctn ).replace( /owner_phone1/g, 'owner_phone' + this.ctn ).replace( /owner_email1/g, 'owner_email' + this.ctn ).replace( /owner_cust1/g, 'owner_cust' + this.ctn ).replace( 'ng-click="addmore_owner();"', 'onclick="s.delmore_owner(this, ' + this.ctn + ')"' ).replace( "plus", "minus" ) + '</div>';
...
app.controller( 'EditCTL', [ '$scope', '$compile', '$http', '$location', '$route', '$routeParams', '$q', '$log', function ( $scope, $compile, $http, $location, $route, $routeParams, $q, $log )
{
    var self = this;
    self.simulateQuery = false;
    self.isDisabled = false;
    ...
    function getProperty( )
    {
        $http.get( "../api/property/" + $routeParams.id ).success( function ( data )
        {
            ...
            if( $scope.form.owner_id )
            {
                $('#input-0').val( $scope.owner ).change();
                //self.querySearch( $scope.owner );
            }
            /*
            ...
            {
                if ( i == 0 ) continue;
                html = '<div class="row" id="row_' + k + '"><div class="col-md-2"></div>' + tmpl.html( ).replace( /owner_name1/g, 'owner_name' + k ).replace( /owner_phone1/g, 'owner_phone' + k ).replace( /owner_email1/g, 'owner_email' + k ).replace( /owner_cust1/g, 'owner_cust' + k ).replace( 'ng-click="addmore_owner();"', 'onclick="s.delmore_owner(this, ' + k + ')"' ).replace( "plus", "minus" ) + '</div>';
    ...
    $scope.addmore_owner = function ( )
    {
        ...
        var ctn = this.ctn;
        html = '<div class="row" id="row_' + this.ctn + '"><div class="col-md-2"></div>' + tmpl.html( ).replace( /owner_name1/g, 'owner_name' + this.ctn ).replace( /owner_phone1/g, 'owner_phone' + this.ctn ).replace( /owner_email1/g, 'owner_email' + this.ctn ).replace( /owner_cust1/g, 'owner_cust' + this.ctn ).replace( 'ng-click="addmore_owner();"', 'onclick="s.delmore_owner(this, ' + this.ctn + ')"' ).replace( "plus", "minus" ) + '</div>';
        $( html ).find( 'div' ).first( ).remove( );
        moreowner.append( $compile( html )( $scope ) );
        moreowner.find( 'div[name=ref_id]' ).each( function ( )
        {
