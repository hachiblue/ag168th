...
var app = angular.module('approver-app', ['ngRoute', 'angular-loading-bar', 'localytics.directives', 'ngMaterial']);
...
app.config(['$routeProvider', 'cfpLoadingBarProvider', '$mdDateLocaleProvider',
    function ($routeProvider, cfpLoadingBarProvider, $mdDateLocaleProvider)
    {
        ...
        when('/',
        {
            templateUrl: '../public/app/approver/index.php'
        }).
        when('/add',
        {
            templateUrl: '../public/app/approver/add.php'
        }).
        when('/edit/:id',
        {
            templateUrl: '../public/app/approver/edit.php'
        }).
        when('/m/:month/y/:year',
        {
            templateUrl: '../public/app/approver/index.php'
...
app.controller('IndexCTL', ['$scope', '$mdDialog', '$compile', '$http', '$location', '$route', '$routeParams', function ($scope, $mdDialog, $compile, $http, $location, $route, $routeParams) {
	...
	$scope.submit = function ()
    {
		...
		$.get("../api/approver", $scope.form, function ( calendar ) {
		...
		//window.open('/admin/approver#/m/'+$scope.form.month+'/y/'+$scope.form.year+'', '_self');
	...
	$scope.showAdvanced = function(ev, m, y, d) {
		$mdDialog.show({
			...
			templateUrl: '../api/approver/whos?y='+y+'&m='+m+'&d='+d,
...
app.controller('AddCTL', ['$scope', '$compile', '$http', '$location', function ($scope, $compile, $http, $location) {
    ...
    {	
		...
		$scope.form.rqshift_approver_total_tm = h + ':' + m;

        $.post("../api/approver", $scope.form, function (data)
        {
            if (data.error)
            {
...
app.controller('EditCTL', ['$scope', '$compile', '$http', '$location', '$routeParams', function ($scope, $compile, $http, $location, $routeParams) {
	...
	{
		$http.get("../api/approver/edit/" + $routeParams.id).success(function (data)
        {
			...
			data.total_time_hours = setHours( data.rqshift_approver_total_tm );
			data.f_minutes = setMinutes( data.rqshift_from_tm );
			data.t_minutes = setMinutes( data.rqshift_to_tm );
			data.total_time_minutes = setMinutes( data.rqshift_approver_total_tm );
    ...
    $scope.submit = function ()
    {
        ...
        $.post("../api/approver/edit/" + $routeParams.id, $scope.form, function (data)
        {
            if (data.error)
            {
                alert(data.error.message);
	...
	$scope.remove = function (id)
    {
        ...
        $http.delete("../api/approver/" + id).success(function (data)
        {
