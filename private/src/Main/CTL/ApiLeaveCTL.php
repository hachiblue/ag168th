...
class ApiLeaveCTL extends BaseCTL
{
    ...
    public function add()
    {
        ...
        if ('2' == $_SESSION['login']['level']['id'] || '4' == $_SESSION['login']['level']['id'] || '5' == $_SESSION['login']['level']['id'])
        {
            $insert['status'] = 1;
        }

        if ('3' == $_SESSION['login']['level']['id'] || '7' == $_SESSION['login']['level']['id'] || '8' == $_SESSION['login']['level']['id'] || '9' == $_SESSION['login']['level']['id'])
        {
            $insert['status']                = 2;
            $insert['supervisor_approve_id'] = $accId;
        }

        if ('6' == $_SESSION['login']['level']['id']) // HR
        {
            $insert['status']                = 3;
            $insert['hr_id']                 = $accId;
            $insert['supervisor_approve_id'] = $accId;
    ...
    public function upload()
    {
        ...
        $validator         = new \FileUpload\Validator\Simple(1024 * 1024 * 4, array('image/png', 'image/jpg', 'image/jpeg', 'image/gif'));
        $pathresolver      = new \FileUpload\PathResolver\Simple('public/file_pics');
        ...
        {
            if (0 == $file->error && $file->name)
            {
                $db->update('mng_leave', array('img' => $file->name), array('id' => $id));
            }
        }
    ...
    public function delete()
    {
        ...
        $db = MedooFactory::getInstance();
        $id = $db->delete('mng_leave', array('id' => $id));
    ...
    public function whos()
    {
        ...
        foreach ($rows as $row)
        {
            ...
            if (strtotime($row['created_at']) > strtotime('-1 days') && 1 == $row['status'])
            {
                echo '
					<a href="#edit/' . $row['id'] . '">
